...
async def _call(session: ClientSession, name: str, arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    if payload["status"] != "ok":
        ...
        if name in {"cad_commit", "cad_drawing_read"}:
...
def _trigger_owned_bridge_load(com: ComAutoCADAdapter) -> None:
    ...
    AutoCAD's ``/Automation -Embedding`` startup does not run package startup loading.
    ...
    bundle_root = (
        ...
        / "Autodesk"
        / "ApplicationPlugins"
        / "AutoCADMechanicalHarness.R26.bundle"
        / "Contents"
        / "Windows"
    ).resolve()
    bridge_dll = (bundle_root / "AutoCADHarness.dll").resolve()
    if not bridge_dll.is_file() or bundle_root not in bridge_dll.parents:
        raise FileNotFoundError(f"Installed R26 bridge DLL is unavailable: {bridge_dll}")
...
def run_acceptance(
    *,
    ...
    os.environ["CAD_HARNESS_CHECKPOINT_ROOT"] = str(case_root / "bridge-checkpoints")
    os.environ["CAD_HARNESS_COMMIT_JOURNAL_ROOT"] = str(case_root / "commit-journal")
    ...
    try:
        ...
        workflow["bridge_preflight"] = workflow["adapter"]
