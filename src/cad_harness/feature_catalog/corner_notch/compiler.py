"""Compiler for a rectangular notch removed from a parent corner."""
...
from cad_harness.geometry.cutouts import corner_notch
from cad_harness.geometry.predicates import contour_overflow_mm


class CornerNotchCompiler:
    feature_type = "corner_notch"
    schema_version = SCHEMA_VERSION
    description = "Rectangular notch removed from a selected corner of a parent outline."
    required_parameters = ("corner_index", "width_mm", "height_mm")
    ...
    def validate_inputs(self, feature: FeatureSpec, context: CompileContext) -> InputReport:
        ...
        report.require(
            integer(feature.parameters, "corner_index") is not None,
            f"{prefix}.corner_index",
            "Target corner index is required",
            "zero-based integer",
        )
        ...
        report.require(
            ...
            "Corner notch must be declared as a child of an outline feature",
            "child feature",
        )
        return report
    ...
    def compile(self, feature: FeatureSpec, context: CompileContext) -> CompiledFeature:
        ...
        if not report.is_complete:
            raise missing_error("Corner notch inputs are incomplete", report)
        assert context.parent_outline is not None
        result = corner_notch(
            context.parent_outline,
            int(feature.parameters["corner_index"]),
            float(feature.parameters["width_mm"]),
            float(feature.parameters["height_mm"]),
            context.tolerance,
        )
        ...
        if not context.tolerance.is_zero_length(overflow):
            raise InvalidGeometryError(
                "Corner notch extends outside its parent outline",
