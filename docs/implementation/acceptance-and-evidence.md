...
latest complete default Python gate is 909 passed with 12 explicit skips: unsafe active-document
COM writes, opt-in disposable AutoCAD acceptance, and opt-in live performance.
Ruff formatting and lint pass, 26 generated schemas are current, and strict type checking
passes across 198 source files. The pure C# bridge suite passes 144/144, the AutoCAD 2027
...
| Complex 2D creation | Ten deterministic feature families, modifiers/annotations/views, 34 complete synthetic semantic cases, and configured performance gates | 30–50 engineer-selected company drawings, real profiles and representative prompt/pilot evidence |
...
| Takeoff and quotation data | Pure takeoff engine, versioned demo material table, mass/cut/pierce/hole/weld quantities, provenance, SQLite audit and five synthetic DXF/takeoff cases | Independently calculated answers on engineer-selected drawings and an approved company material table |
| Audit and modification | Deterministic auditor plus persisted-evidence remediation compiler; exact selected-finding plans, semantic deletion refs, preview/approval/commit gates, stale blocking, durable checkpoint metadata, separate rollback approval, full post-commit re-audit and live session undo | Live update/delete remediation, durable checkpoint-file replacement and engineer acceptance |
| Measurement | Twelve analytic measurements, cooperative terminal timeouts, provenance/revision/tolerance, properties and schemas | Real AutoCAD readback comparison |
...
| MCP/client experience | 22 typed tools, fail-closed permissions, PySide6 engineer approval, global local-MCP install, actual Codex discovery and fake pipeline through `VALIDATED`; schema 1.10 status smoke passed | Restart desktop to load the installed server; ChatGPT web requires a remote MCP/tunnel; real-AutoCAD client acceptance remains open |
